...
The check also requires the exact minimum from `engines.node` to appear in the
CI matrix. When the executable test command needs a newer runtime, update the
package metadata, CI matrix, and documented minimum together.
